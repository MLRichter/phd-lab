...
def _obtain_savefile(data_file_name: str, target_file_name: str) -> Tuple[Path, Path]:
    pkl_file = Path(data_file_name.replace("train-", "").replace(".p", "") + "_" + target_file_name.replace(".csv", ".pkl"))
    png_file = Path(data_file_name.replace("train-", "").replace(".p", "") + "_" + target_file_name.replace(".csv", ".png"))
...
def main(args: PseudoArgs):
    ...
    if args.mp != 0:
        ...
        plot_confusion_matrices(cms, eval_datasets, config.PROBE_PERFORMANCE_SAVEFILE)
        pd.DataFrame.from_dict(
            {
                'name': names,
                'train_acc': t_accs,
